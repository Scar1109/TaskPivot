...
import com.example.taskpivot.modules.PriorityScreen

class PriorityDeleteConformationModel : BottomSheetDialogFragment() {

    companion object {
        const val TAG = "PriorityDeleteConformationModel"
        const val ARG_TASK_ID = "task_id"

        fun newInstance(taskId: Int): PriorityDeleteConformationModel {
            val args = Bundle().apply {
                putInt(ARG_TASK_ID, taskId)
            }
            return PriorityDeleteConformationModel().apply {
                arguments = args
            }
        }
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        remove_btn.setOnClickListener {
            ...
            if (deletedRows > 0) {
                ...
                (requireActivity() as PriorityScreen).deleteTaskAndRefresh(taskId)
            } else {
